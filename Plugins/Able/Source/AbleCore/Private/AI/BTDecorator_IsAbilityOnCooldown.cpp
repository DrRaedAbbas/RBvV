...
const UAblAbility* UBTDecorator_IsAbilityOnCooldown::GetAbility(const UBlackboardComponent* BlackboardComp) const
{
...
FString UBTDecorator_IsAbilityOnCooldown::GetStaticDescription() const
{
    ...
    return FString::Printf(TEXT("%s: checks if Ability [%s] is on cooldown."), *Super::GetStaticDescription(), *AbilityName);
}
